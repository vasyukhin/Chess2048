...
function getDifficultyConfig() {
  if (difficulty === "easy") return { depth: 1, thinkMs: 180, randomizeTop: 3 };
  if (difficulty === "hard") return { depth: 3, thinkMs: 420, randomizeTop: 1 };
  return { depth: 2, thinkMs: 280, randomizeTop: 2 };
}
...
function pickComputerMove() {
  ...
  const { depth, randomizeTop } = getDifficultyConfig();
  ...
  const bucketSize = Math.max(1, Math.min(randomizeTop, scoredMoves.length));
  const bucket = scoredMoves.slice(0, bucketSize);
  const chosen = bucket[Math.floor(Math.random() * bucket.length)]?.move ?? scoredMoves[0]?.move;
  return chosen ?? null;
}
...
function move(from, to) {
  ...
  const movingPiece = getPiece(from);
  const targetPiece = getPiece(to);
  history.push(JSON.stringify(state));
  state = applyMove(state, from, to);
  return { from, to, movingPiece, targetPiece };
}

function animateMove(moveData, onDone = () => {}) {
  if (!moveData) {
    renderBoard();
    onDone();
    return;
  ...
  const fromEl = boardEl.querySelector(`[data-square="${moveData.from}"]`);
  const toEl = boardEl.querySelector(`[data-square="${moveData.to}"]`);
...
function maybeComputerTurn() {
  ...
  const { thinkMs } = getDifficultyConfig();

  setTimeout(() => {
    const best = pickComputerMove();
    const moved = best ? move(best.from, best.to) : null;
...
function updateStatus() {
  ...
  const levelLabel = difficulty === "easy" ? "лёгкий" : difficulty === "hard" ? "сложный" : "средний";
  statusEl.textContent = `${side}${computerHint} ходят${isCheck(state.turn, state) ? ", шах." : "."} Уровень: ${levelLabel}.`;
}

function onSquareClick(event) {
  if (state.turn !== HUMAN_COLOR || isComputerThinking || isAnimatingMove) return;

  const square = event.currentTarget.dataset.square;

  if (selectedSquare && legalTargets.includes(square)) {
    const moved = move(selectedSquare, square);
    selectedSquare = null;
    legalTargets = [];
    animateMove(moved, maybeComputerTurn);
    return;
  }
...
undoBtn.addEventListener("click", () => {
  if (isComputerThinking || isAnimatingMove) return;
